...
% System boundary
\node[draw, dashed, rounded corners=8pt, minimum width=10cm, minimum height=10cm,
      label={[font=\large\bfseries]above:VeriFi System}] (sys) at (6.5,0) {};

% Actors (left side)
\node[actor] (predictor) at (-1.5, 2.5)  {Predictor};
...
% Use cases
\node[usecase] (register) at (4, 4.5)   {Register Wallet};
\node[usecase] (login)    at (9, 4.5)   {Login (Challenge--Response)};
\node[usecase] (create)   at (4, 2.5)   {Create Hard Claim};
\node[usecase] (sign)     at (9, 2.5)   {Sign Claim};
\node[usecase] (feed)     at (4, 0.5)   {Browse Claim Feed};
\node[usecase] (profile)  at (9, 0.5)   {View Profile \& Truth Score};
\node[usecase] (download) at (4, -1.5)  {Download Proof};
\node[usecase] (verify)   at (9, -1.5)  {Verify Proof};
\node[usecase] (resolve)  at (9, -3.5)  {Resolve Claim};
\node[usecase] (extract)  at (4, -3.5)  {LLM Claim Extraction};
