...
Model G  =  CPMM payouts + v2 hard rules + energy gate
            ...
            + zero-sum cap at resolution:
              if winners would receive more rep than total stakes,
              scale their gains down so total mint = 0
```

Locked reward, copy-trade immunity, whale rules, energy gate — all unchanged from F. No claim is ever cancelled.
...
**Reading.** Creator's expected rep / claim drops by 2.47 when we remove the auto-YES — but they were earning that as a freebie, regardless of claim quality.  Under G, the creator earns when their skill-based vote is correct, which is the honest signal.
...
| 10 sybils, 1 honest NO | +52.24 | **-0.00** | +62.24 | **+10.00** | -10.00 |
| 10 sybils, 5 honest NO | +12.24 | **-0.00** | +62.24 | **+50.00** | -50.00 |
...
- With 1 honest dissenter, attacker gains **10.00 rep** in total — exactly the dissenter's 10 rep, transferred but not minted.
