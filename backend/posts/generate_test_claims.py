"""
Standalone script to generate historical `HardClaim`s for testing.

This initializes Django so the ORM can be used outside of `manage.py`.
"""
...
from posts.models import Asset, HardClaim, HardClaimEvent, Post
...
def generate_and_save_historical_claims():
    ...
    for symbol in ASSET_SYMBOLS:
        ...
        try:
            ...
            peak_bullish = fetch_peak_price(
                instrument, two_months_ago, one_month_ago, "bullish"
            )
            peak_bearish = fetch_peak_price(
                instrument, two_months_ago, one_month_ago, "bearish"
            )
        ...
        post_1 = Post.objects.create(
            author=author,
            content=(
                f"I'm {direction_1} on {symbol} — expecting at least {target_pct_1}% move."
            ),
        )
        ...
        print(
            f"Saved Correct Claim with ID {claim_1.id} (Post {post_1.id}): "
            f"{symbol} {direction_1} {target_pct_1}%"
        )
        ...
        post_2 = Post.objects.create(
            author=author,
            content=(
                f"Calling a big {direction_2} move on {symbol} — "
                f"at least {target_pct_2}% upside."
            ),
        )
        ...
        print(
            f"Saved Incorrect Claim with ID {claim_2.id} (Post {post_2.id}): "
            f"{symbol} {direction_2} {target_pct_2}%"
        )
        print("-" * 40)
