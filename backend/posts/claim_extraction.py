...
def _detect_quarter(lowered: str) -> Optional[int]:
    ...
    """
    m = re.search(r"\bq\s{0,20}([1-4])\b", lowered) or re.search(r"\bç\s{0,20}([1-4])\b", lowered)
    if m:
        return int(m.group(1))
    m = re.search(r"\b([1-4])\s{0,20}\.?\s{0,20}(?:çeyre|quarter)", lowered)
    if m:
        return int(m.group(1))
    ordinals = [
...
def extract_deadline(text: str, base_date: Optional[datetime] = None) -> Optional[str]:
    ...
    md = re.search(
        rf"\b({_MONTH_ALT})\.?\s+(\d{{1,2}})(?:st|nd|rd|th)?\b(?!\s{{0,20}}%)(?:,?\s{{0,20}}(20\d{{2}}))?",
        lowered,
    )
    if md:
    ...
    dm = re.search(
        rf"\b(\d{{1,2}})(?:st|nd|rd|th)?\s+(?:of\s+)?({_MONTH_ALT})\b(?:,?\s{{0,20}}(20\d{{2}}))?",
        lowered,
    )
    if dm:
    ...
    if re.search(r"\b(?:bugün|bugun)\w{0,12}|\btoday\b", lowered):
        return now.strftime("%Y-%m-%d")
    if re.search(r"\b(?:yarın|yarin)\w{0,12}|\btomorrow\b", lowered):
    ...
    wk = re.search(r"(\d{1,20})\s{0,20}(?:hafta|week|weeks)\b", lowered)
    if wk:
        return (now + timedelta(weeks=int(wk.group(1)))).strftime("%Y-%m-%d")
    dy = re.search(r"(\d{1,20})\s{0,20}(?:gün|gun|day|days)\b", lowered)
    if dy:
        return (now + timedelta(days=int(dy.group(1)))).strftime("%Y-%m-%d")
    mo_ = re.search(r"(\d{1,20})\s{0,20}(?:ay|month|months)\b", lowered)
    if mo_:
        return (now + timedelta(days=int(mo_.group(1)) * 30)).strftime("%Y-%m-%d")
    yr = re.search(r"(\d{1,20})\s{0,20}(?:yıl|yil|year|years)\b", lowered)
    if yr:
    ...
    if re.search(
        r"\b(?:hafta\w{0,12}|gelecek hafta|önümüzdeki hafta|onumuzdeki hafta)\b|\bnext week\b",
        lowered,
    ):
        return (now + timedelta(days=7)).strftime("%Y-%m-%d")
    if re.search(
        r"\b(?:gelecek|önümüzdeki|onumuzdeki)\s+ay\w{0,12}|\bnext month\b",
        lowered,
    ):
        return (now + timedelta(days=30)).strftime("%Y-%m-%d")
    if re.search(
        r"\b(?:gelecek|önümüzdeki|onumuzdeki|ertesi)\s+yıl\w{0,12}|\bnext year\b",
        lowered,
    ):
        return (now + timedelta(days=365)).strftime("%Y-%m-%d")
    ...
    verbal = re.search(r"\b(20\d{2})\s{0,20}(baş\w{0,12}|son\w{0,12}|yıl\w{0,12}|için\w{0,12}|icin\w{0,12})", lowered)
    if verbal:
        year = verbal.group(1)
...
def map_asset_token(token: str, allow_multi_token: bool = True, allow_fuzzy: bool = True) -> Optional[str]:
    ...
    if allow_fuzzy:
        close = get_close_matches(normalized, list(alias_map.keys()), n=1, cutoff=0.85)
...
def _extract_pair_assets(text: str) -> tuple[Optional[str], Optional[str]]:
    pair = re.search(r"([A-Za-zÇĞİÖŞÜçğıöşü]{2,20})\s{0,20}/\s{0,20}([A-Za-zÇĞİÖŞÜçğıöşü]{2,20})", text)
    if not pair:
        return None, None
...
_NUMBER_TOKEN_RE = re.compile(
    rf"(?P<num>\d[\d.,]*\d|\d)\s{{0,20}}(?P<mult>{_MULT_ALT})?\b",
    re.IGNORECASE,
)
...
def _in_period_context(text: str, start: int, end: int) -> bool:
    ...
    if re.search(r"(?:[qç]\s{0,20}|h)$", before):
        return True
    if re.match(r"\s{0,20}(?:h\b|\.?\s{0,20}çeyre|\.?\s{0,20}quarter|\.?\s{0,20}yar[ıi]|\.?\s{0,20}half)", after):
        return True
    return False
...
def _number_is_percentage(text: str, start: int, end: int) -> bool:
    ...
    if re.search(r"%\s{0,20}$", before) or re.search(r"(?:y[uü]zde|percent)\w{0,12}\s{0,20}$", before):
        return True
    if re.match(r"\s{0,20}%|\s{0,20}percent", after):
        return True
    return False
...
def _extract_value_with_payda(
    ...
    for value, start, end in _scan_number_groups(text):
        ...
        if _number_is_percentage(text, start, end):
            continue
        token_match = re.match(r"\s{0,20}([A-Za-zÇĞİÖŞÜçğıöşü$€₺]{2,15})", text[end:])
        if not token_match:
            continue
...
def _extract_percentage_value(text: str) -> tuple[Optional[float], int]:
    ...
    percent_patterns = [
        r"%\s{0,20}(\d{1,20}(?:[.,]\d{1,20})?)",
        r"(\d{1,20}(?:[.,]\d{1,20})?)\s{0,20}%",
        r"y[uü]zde\s{0,20}(\d{1,20}(?:[.,]\d{1,20})?)",
        r"(\d{1,20}(?:[.,]\d{1,20})?)\s{0,20}percent",
    ]
    lowered = text.lower()
...
def _extract_base_payda_from_text(text: str) -> Optional[str]:
    ...
    base_patterns = [
        ...
        r"vs\.?\s{0,20}([A-Za-zÇĞİÖŞÜçğıöşü]{2,15})",
    ]
...
_CLAUSE_SPLIT_RE = re.compile(
    r"(?<!\d)[.,](?!\d)|;|\b(?:ve|veya|fakat|ama|ancak|and|or|but)\b",
    re.IGNORECASE,
)
...
def _find_locked_pairs(text: str) -> List[dict]:
    ...
    for m in re.finditer(
        r"([A-Za-zÇĞİÖŞÜçğıöşü]{2,20})\s{0,20}[/\-]\s{0,20}([A-Za-zÇĞİÖŞÜçğıöşü]{2,20})",
        text,
    ):
...
def _scan_deadline_mentions(text: str, base_date: Optional[datetime] = None) -> List[dict]:
    ...
    def _lead_int(span: str) -> int:
        return int(re.match(r"\d{1,20}", span).group(0))

    for m in re.finditer(r"\d{1,20}\s{0,20}(?:hafta\w{0,12}|weeks?)\b", lowered):
        add(m.start(), m.end(), (now + timedelta(weeks=_lead_int(m.group(0)))).strftime("%Y-%m-%d"))
    for m in re.finditer(r"\d{1,20}\s{0,20}(?:gün\w{0,12}|gun\w{0,12}|days?)\b", lowered):
        add(m.start(), m.end(), (now + timedelta(days=_lead_int(m.group(0)))).strftime("%Y-%m-%d"))
    for m in re.finditer(r"\d{1,20}\s{0,20}(?:ay\w{0,12}|months?)\b", lowered):
        add(m.start(), m.end(), (now + timedelta(days=_lead_int(m.group(0)) * 30)).strftime("%Y-%m-%d"))
    for m in re.finditer(r"\d{1,20}\s{0,20}(?:yıl\w{0,12}|yil\w{0,12}|years?)\b", lowered):
    ...
    for m in re.finditer(r"\b(?:bugün|bugun)\w{0,12}|\btoday\b", lowered):
        add(m.start(), m.end(), now.strftime("%Y-%m-%d"))
    for m in re.finditer(r"\b(?:yarın|yarin)\w{0,12}|\btomorrow\b", lowered):
    ...
    # 5) Year-end / EOY ----------------------------------------------------
    for m in re.finditer(
        r"\beoy\b|yıl sonu\w{0,12}|yil sonu\w{0,12}|sene sonu\w{0,12}|yıl sonuna kadar|"
    ...
    # 6) End of month / EOM ------------------------------------------------
    for m in re.finditer(r"\beom\b|ay sonu\w{0,12}|month[- ]end|end of (?:the )?month", lowered):
    ...
    for m in re.finditer(r"\bq\s{0,20}([1-4])\b|\b([1-4])\s{0,20}\.?\s{0,20}(?:çeyre\w{0,12}|quarter\w{0,12})", lowered):
        q = int(m.group(1) or m.group(2))
        year = _year_in_text(lowered, base_date)
        mo, d = _QUARTER_END[q]
        add(m.start(), m.end(), _iso(year, mo, d))
    for pattern, q in (
        (r"\b(?:ilk çeyre\w{0,12}|first quarter|1st quarter)", 1),
        (r"\b(?:ikinci çeyre\w{0,12}|second quarter|2nd quarter)", 2),
        (r"\b(?:üçüncü çeyre\w{0,12}|uçuncu çeyre\w{0,12}|third quarter|3rd quarter)", 3),
        (r"\b(?:dördüncü çeyre\w{0,12}|dorduncu çeyre\w{0,12}|fourth quarter|4th quarter)", 4),
    ):
    ...
    for m in re.finditer(
        r"\b(?:ilk yar[ıi]\w{0,12}|yılın ilk yar[ıi]\w{0,12}|first half|1st half)", lowered
    ):
        add(m.start(), m.end(), _iso(_year_in_text(lowered, base_date), 6, 30))
    for m in re.finditer(
        r"\b(?:ikinci yar[ıi]\w{0,12}|yılın ikinci yar[ıi]\w{0,12}|second half|2nd half)", lowered
    ):
        add(m.start(), m.end(), _iso(_year_in_text(lowered, base_date), 12, 31))

    # 11) Verbal year phrases ("2027 başında", "2028 sonu") ----------------
    for m in re.finditer(r"\b(20\d{2})\s{0,20}(baş\w{0,12}|son\w{0,12}|yıl\w{0,12}|için\w{0,12}|icin\w{0,12})?\b", lowered):
        year = int(m.group(1))
